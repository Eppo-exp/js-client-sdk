...
describe('EppoJSClient E2E test', () => {
  ...
  afterEach(() => {
    // Reset the UFC response function to its original state
  ...
  //     td.replace(
  //       LocalStorageBackedAsyncStore.prototype,
  //       'getEntries',
  //       (key: string) => null as null,
  //     );
  ...
  describe('UFC Test Cases', () => {
    [true, false].forEach((obfuscated) => {
      describe(`${obfuscated ? 'Obfuscated' : 'Not Obfuscated'}`, () => {
        beforeAll(async () => {
          global.fetch = jest.fn(() => {
            const ufc = readMockUfcResponse(
              obfuscated ? OBFUSCATED_MOCK_UFC_RESPONSE_FILE : MOCK_UFC_RESPONSE_FILE,
            );
